...
&nbsp;&nbsp;이를 해결하기 위한 자료구조가 '[힙(Heap)]()'이라는 것이다. 이는 제거 연산까지 알아본 후에 자세히 다뤄보도록 한다.
...
> 그림에서도 보이듯 힙의 구조를 트리로써 표현했다. 예전 [포스팅](/_posts/data-structure/2021-03-30-Tree(Binary).md)에서 이진 트리를 구현할 때는 링크드 리스트를 기반으로 두었었다. 하지만 링크드 리스트 기반의 구현은 힙의 가장 마지막노드(최고 깊이, 최 우측노드)를 찾는 효율적인 방법을 찾기가 쉽지 않다.
